...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        NavigableMap<LocalDate,List<Purchase>> datedPurcheses = new TreeMap<>();

        for (Purchase p : purchases.values() ) {
            datedPurcheses.compute(p.purchaseDate(),
                    (pdate, plist) -> {
                        List<Purchase> list =
        ...
        datedPurcheses.forEach((key, value) -> System.out.println(key + ": " + value));

        int currentYear = LocalDate.now().getYear();

        LocalDate firstDay = LocalDate.ofYearDay(currentYear,1);
        LocalDate week1 = firstDay.plusDays(7);
        Map<LocalDate, List<Purchase>> week1Purchases = datedPurcheses.headMap(week1);
        Map<LocalDate, List<Purchase>> week2Purchases = datedPurcheses.tailMap(week1);
...
//        week2Purchases.forEach((key, value) -> System.out.println(key + ": " + value));
        ...
        LocalDate lastDate = datedPurcheses.lastKey();
        var previousEntry = datedPurcheses.lastEntry();
        ...
        while (previousEntry != null) {
            ...
            LocalDate prevDate = datedPurcheses.lowerKey(lastDate);
            previousEntry = datedPurcheses.lowerEntry(lastDate);
            lastDate = prevDate;
        }

        System.out.println("----------------------------------------");
        var reversed = datedPurcheses.descendingMap();
